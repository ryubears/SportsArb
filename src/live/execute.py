...
MAX_QUANTITY = 500          # Contracts per trade. Both legs together cost about a dollar a contract, so this caps a trade near 500 dollars.
...
class PaperExecutor:
    ...
    def signal(self, pair, yes, no, edge, size, fee_infos, now):
        ...
        The cost is reserved here, before anything is awaited, so a second
        ...
        quantity = int(min(size * FILL_SHARE, MAX_QUANTITY, *(self.cash[l["member"]["venue"]] // l["limit"] for l in legs)))
        if quantity < 1:
            return False
        for l in legs:
            l["quantity"] = quantity
    ...
    async def run_trade(self, trade, legs):
        ...
        if yes_fill.filled != no_fill.filled:
            await self.flatten(trade, legs, fills)
    ...
    async def flatten(self, trade, legs, fills):
        """
