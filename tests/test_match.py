...
def test_same_bet_on_two_venues_forms_one_group():
    groups, unmatched = match.match([bet("polymarket_us", "us"), bet("kalshi", "k")])
...
def test_bets_on_a_single_venue_are_left_out():
    groups, unmatched = match.match([bet("polymarket_us", "us"), bet("kalshi", "k", subject="MIA")])
    assert groups == [] and len(unmatched) == 2


def test_far_apart_close_times_are_flagged():
    groups, _ = match.match([bet("polymarket_us", "us", close_time="2027-03-31T23:55:00+00:00"),
                             bet("kalshi", "k", close_time="2029-02-13T23:30:00+00:00")])
    assert any(f.startswith("close times") for f in groups[0].flags)
    assert any(f.startswith("close times") for f in groups[0].flags)


def test_winner_group_holds_both_kalshi_contracts_and_the_away_market():
    game = dict(kind="game_winner", game_date="2026-09-20", team_a="CAR", team_b="ATL", subject="CAR")
    groups, _ = match.match([bet("polymarket_us", "us_car", polarity="yes", **game),
                             bet("kalshi", "k_car", polarity="yes", **game),
                             bet("kalshi", "k_atl", polarity="no", **game)])
    assert sorted((m.contract_id, m.polarity) for m in groups[0].members) == [("k_atl", "no"), ("k_car", "yes"), ("us_car", "yes")]
