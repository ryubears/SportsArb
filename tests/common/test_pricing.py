...
def test_fill_walks_both_ladders_while_the_edge_is_positive():
    ...
    top_edge, size, profit = pricing.fill(leg_a, leg_b, "polymarket", "polymarket", fee_infos)
...
def test_fill_stops_at_zero_edge_and_handles_empty_ladders():
    fee_infos = {"polymarket": NO_PM_FEES}
    assert pricing.fill([(0.5, 10)], [(0.5, 10)], "polymarket", "polymarket", fee_infos) == (0.0, 0.0, 0.0)
    assert pricing.fill([], [(0.5, 10)], "polymarket", "polymarket", fee_infos) == (-1.0, 0.0, 0.0)
...
def test_best_trade_ignores_a_crossed_book_with_no_partner():
    ...
    assert pricing.best_trade(members, quotes, {("polymarket", "pm"): NO_PM_FEES}) is None
